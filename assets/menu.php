<nav class="menu">
    <a href="index.php">
        <img class="logo" src="assets/img/logo.svg" alt="" />
    </a>
        
    <a href="javascript:void(0)" class="MobileItem" id="abrirmenu">
        <span id="Cima"></span>
        <span id="Meio"></span>
        <span id="Baixo"></span>
    </a>

    <div class="links">

        <ul>
            <a class="link-abre active" href="index.php">DNA</a>
        </ul>

        <ul>
            <a class="link-abre" href="logotipo.php">Logotipos & Símbolo</a>

            <li>
                <a href="#sessao2">Arquitetura</a>
            </li>
            <li>
                <a href="#sessao3">Versões de marca</a>
            </li>
            <li>
                <a href="#sessao4">Redução máxima</a>
            </li>
            <li>
                <a href="#sessao5">Área de proteção</a>
            </li>
            <li>
                <a href="#sessao6">Aplicação sobre cores e fundos</a>
            </li>
            <li>
                <a href="#sessao7">Aplicação escrita</a>
            </li>
            <li>
                <a href="#sessao8">Usos incorretos</a>
            </li>
            <li>
                <a href="#sessao9">Campanhas e Programas internos</a>
            </li>
            <li>
                <a href="#sessao13">Símbolo</a>
            </li>
            <li>
                <a href="#sessao15">Para se inspirar</a>
            </li>
        </ul>

        <ul>
            <a class="link-abre" href="paleta.php">Paleta cromática</a>

            <li>
                <a href="#sessao1">Institucional</a>
            </li>
            <li>
                <a href="#sessao2">Subtons de Apoio</a>
            </li>
            <li>
                <a href="#sessao3">Proporção de uso</a>
            </li>
            <li>
                <a href="#sessao4">Cor + texto</a>
            </li>
        </ul>

        <ul>
            <a class="link-abre" href="tipografia.php">Tipografia</a>

            <li>
                <a href="#sessao1">Diretrizes</a>
            </li>
            <li>
                <a href="#sessao2">Hierarquia</a>
            </li>
            <li>
                <a href="#sessao3">Acessibilidade</a>
            </li>
        </ul>

        <ul>
            <a class="link-abre" href="iconografia.php">Iconografia</a>

            <li>
                <a href="#sessao1">Deck</a>
            </li>
            <li>
                <a href="#sessao2">Boas práticas</a>
            </li>
            <li>
                <a href="#sessao3">Sobre fundos coloridos</a>
            </li>
            <li>
                <a href="#sessao4">O que evitar</a>
            </li>
            <li>
                <a href="#sessao5">Para se inspirar</a>
            </li>
        </ul>

        <ul>
            <a class="link-abre" href="grafismos.php">Grafismos</a>

            <li>
                <a href="#sessao2">Composição</a>
            </li>
            <li>
                <a href="#sessao3">Proporção</a>
            </li>
            <li>
                <a href="#sessao4">Orientação</a>
            </li>
            <li>
                <a href="#sessao5">Nº de divisões</a>
            </li>
            <li>
                <a href="#sessao6">Combinação de cores</a>
            </li>
            <li>
                <a href="#sessao7">Interação com produto</a>
            </li>
            <li>
                <a href="#sessao8">Sólidos</a>
            </li>
            <li>
                <a href="#sessao9">Usos incorretos</a>
            </li>
            <li>
                <a href="#sessao10">Para inspirar</a>
            </li>
        </ul>

        <ul>
            <a class="link-abre" href="estilo-fotografico.php">Estilo fotográfico</a>
        </ul>

        <ul>
            <a class="link-abre" href="aplicacoes.php">Aplicações</a>
        </ul>


        <!-- <ul>
                    <a class="identidade link-abre" href="identidade.php">Identidade Verbal</a>

                    <li>
                        <a href="#sessao1">Ponto de partida</a>
                    </li>
                    <li>
                        <a href="#sessao2">Tom de voz</a>
                    </li>                    
                </ul> -->


        <!-- <a class="identidade" href="identidade.php">Identidade Verbal</a> -->


        <div class="navfooter">
            <a href="downloads.php">Central de downloads</a>

            <a href="index.php">Sair</a>
        </div>
    </div>


</nav>